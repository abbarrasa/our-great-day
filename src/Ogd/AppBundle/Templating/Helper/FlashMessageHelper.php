...
class FlashMessageHelper extends Helper
{
    ...
    public function getFlashMessage($type, $title, $message, $parameters = array(), $translationDomain = null)
    {
        return [
            'alert'   => $this->getAlertClassName($type),
            'icon'    => $this->getIconClassName($type),
            'title'   => $this->translator->trans($title, [], $translationDomain),
            'message' => $this->translator->trans($message, $parameters, $translationDomain)
        ];
    }
