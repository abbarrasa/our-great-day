...
class GoogleMapType extends AbstractType
{
    ...
    const DEFAULT_LONGITUDE  = -0.1245;
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $builder
            ->add($options['lat_name'], TextType::class, array_merge($options['options'], $options['lat_options']))
            ->add($options['lng_name'], TextType::class, array_merge($options['options'], $options['lng_options']));

        // Optional Address field
        if (isset($options['addr_name']) && $options['addr_name']) {
            $builder->add($options['addr_name'], TextType::class, array_merge($options['options'], $options['addr_options']));
        }
    ...
    public function configureOptions(OptionsResolver $resolver)
    {
        $resolver
            ->setDefaults([
                ...
                'map_width'        => self::DEFAULT_MAP_WIDTH, // the width of the map
                'map_height'       => self::DEFAULT_MAP_HEIGHT, // the height of the map
                'default_lat'      => self::DEFAULT_LATITUDE, // the starting position on the map
                'default_lng'      => self::DEFAULT_LONGITUDE, // the starting position on the map
